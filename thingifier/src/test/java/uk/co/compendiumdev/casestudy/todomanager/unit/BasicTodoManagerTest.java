...
public class BasicTodoManagerTest {
    ...
    public void todoModelDefinitionCheck(){
        ...
        EntityDefinition todo = todoManager.getERmodel().getSchema().
    ...
    public void relationshipDefinitionCheck(){
        ...
        EntityDefinition todo = todoManager.getERmodel().getSchema().
                getDefinitionWithSingularOrPluralNamed("todo");
        EntityDefinition project = todoManager.getERmodel().getSchema().
    ...
    public void createAndAmendSomeTodos(){
        ...
        EntityDefinition todos = todoManager.getERmodel().getSchema().
                getDefinitionWithSingularOrPluralNamed("todo");

        EntityInstance tidy = repository.createInstance(
                EntityInstanceDraft.forEntity(todos).
                        withField("title", "Tidy up my room").
                        withField("description", "I need to tidy up my room because it is a mess"));

        EntityInstance paperwork = repository.createInstance(
                EntityInstanceDraft.forEntity(todos).
                        withField("title", "Do Paperwork").
                        withField("description", "Scan everything in, upload to document management system and file paperwork"));

        Assertions.assertEquals("false", paperwork.getFieldValue("doneStatus").asString());

        System.out.println(todoManager.toString());

        tidy = repository.patchInstance(
                tidy,
                EntityInstanceDraft.forEntity(todos).withField("doneStatus", "true"));
    ...
    public void createAndDeleteTodos(){
        ...
        EntityInstance foundit = repository.findInstanceByPrimaryKey(todos, tidy.getPrimaryKeyValue());
