...
public class DefaultGuiRoutings {
    ...
    // we support a top level path e.g. /apichallenges/gui, /simpleapi/gui because we should support
    // multiple GUI views with multiple thingifiers
        ...
        get(
                ...
                (request, response) -> {
                    ...
                    return htmlPages.getInstanceDetailsPage(
                            database, entityName, instanceQueryParams);
                });

        return this;
    }

    // TODO: this is where we would 'inject' or handle the authentication process - could be
    // injected with a Thingifier specific handler
