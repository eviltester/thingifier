...
import uk.co.compendiumdev.thingifier.adapter.httpserver.messagehooks.ClearDataPreHttpRequestHook;
import uk.co.compendiumdev.thingifier.adapter.httpserver.messagehooks.LogTheHttpRequestHook;
import uk.co.compendiumdev.thingifier.adapter.httpserver.messagehooks.LogTheResponseHook;
import uk.co.compendiumdev.thingifier.adapter.httpserver.routehandlers.ShutdownRouteHandler;
...
public class MainImplementation implements AutoCloseable {
    ...
    private HttpRouteRegistry routeRegistry;
    private JavalinHttpServer httpServer;
    ...
    public MainImplementation() {
        ...
        proxyport = 4567; // default local port
    ...
    public void configurePortAndDefaultRoutes() {
        routeRegistry = new HttpRouteRegistry();
        HttpRouteRegistry.use(routeRegistry);
    }
    ...
    public void addBuiltInArgConfiguredHooks() {
        ...
        if (clearDataPeriodically) {
            restServer.registerPreRequestHook(
                    new ClearDataPreHttpRequestHook(clearDownMinutes, thingifier));
        }

        if (verboseMode) {
            restServer.registerPreRequestHook(new LogTheHttpRequestHook());
