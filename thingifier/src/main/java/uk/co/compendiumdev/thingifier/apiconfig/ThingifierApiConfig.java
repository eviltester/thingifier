...
public class ThingifierApiConfig {
    ...
    // the api endpoint url prefix e.g. /api for /api/todos or /apichallenges/api for
    // /apichallenges/api/todos etc.
    private String apiEndpointPrefix;

    // Params
    // todo: willAllowFilteringThroughUrlParams  true/false (default: true)
    // todo: willEnforceFilteringThroughUrlParams true/false ie. 404 error if params when not
    ...
    // Plurals
    // todo: enforcePluralsInApiCalls true/false i.e. throw404ErrorIfWrongPluralSingluarUsed default
    // is true
    ...
    // todo: willApiAllowXMLRequests i.e. process when XML content submitted, if not then throw
    // error (default: true)
    ...
    // todo: willApiAllowJsonRequests i.e. process when Json content submitted, if not then throw
    // error

    // willApiEnforceAcceptHeaderForResponses i.e. respond with 406 if asked for but not allowed XML
