...
public class BodyRelationshipValidator {
    ...
    public ValidationReport validate(
        ...
        for (Map.Entry<String, String> complexKeyValue : fullargs) {
            ...
            if (complexKey.startsWith("relationships.")) {
                if (!validateComplexFourPartRelationshipDefinition(
                        thingDefinition,
                        report,
                        complexKey,
                ...
                if (complexKey.contains(".")) {
                    ...
                    if (thingDefinition.related().hasRelationship(parts[0])) {
                        validRelationships =
                                validateCompressedRelationshipDefinition(
                                        thingDefinition,
                                        report,
                                        complexKey,
                                        complexKeyValue.getValue(),
    ...
    private boolean validateCompressedRelationshipDefinition(
        ...
        if (!linkingFields.contains(fieldToMatchForGuid)) {
            report.addErrorMessage(
                    String.format(
                            "Do not support relationship references using %s",
                            fieldToMatchForGuid));
            return false;
        }

        EntityInstance thingToRelateTo = thingifier.findThingInstanceByGuid(guidValue, database);
        // if we cannot find it by a guid then we need to identify the relationship type and find it
        // by id for things
        if (thingToRelateTo == null) {
            final List<RelationshipVectorDefinition> relationshipsNamed =
                    thingDefinition.related().getRelationships(relationShipName);
    ...
    private boolean validateComplexFourPartRelationshipDefinition(
        ...
        if (!validRelationshipBetweenThings(
                thingDefinition, relationshipNamePart, relationshipToPart, report)) {
            return false;
        }
    ...
    private void reportIsNotValidRelationship(
            final String relationshipToMention, ValidationReport report) {
        report.addErrorMessage(
    ...
    private boolean validRelationshipBetweenThings(
        ...
        for (RelationshipVectorDefinition relationships :
                thingDefinition.related().getRelationships(relationShipName)) {
            if (relationships.getTo().getPlural().equals(thingToRelateTo)) {
                return true;
            }
        }
