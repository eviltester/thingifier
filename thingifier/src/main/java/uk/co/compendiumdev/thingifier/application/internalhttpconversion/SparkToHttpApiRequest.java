...
public class SparkToHttpApiRequest {
    ...
    public static HttpApiRequest convert(final Request request) {
        HttpApiRequest apiRequest =
                new HttpApiRequest(request.pathInfo())
                        ...
                        .setFilterableQueryParams(request.queryString())
                        . // use our own parser to get params
                        setVerb(request.requestMethod())
                        .setUrl(request.url())
                        .setIP(request.ip())
                        .
                        // this is a standard parser and splits on = it does not parse as we need
                        // for sorting, filtering etc. e.g. id>=17 becomes id> 17 not id >=17
                        setUrlParams(request.params())
                        .
                        // the default Spark headers is a map so filters out duplicates, allow
                        // working with the raw list when necessary
    ...
    private static Map<String, String> rawQueryParamsAsMap(final Request request) {
        ...
        for (String paramName : request.queryParams()) {
            // todo: figure out what to do if more than one in each value, currently we lose the
            // values
