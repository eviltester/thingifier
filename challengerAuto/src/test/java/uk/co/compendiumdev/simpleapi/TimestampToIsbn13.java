...
/**
 * Utility that generates an ISBN‑13‑style string out of the current time (epoch milliseconds).
 *
 * <p>The format is {@code XXX-X-XX-XXXXXX-X} (e.g. “999‑9‑99‑123456‑2”). The digits come from the
 * 13‑digit representation of {@code System.currentTimeMillis()}.
 *
 * <pre>{@code
...
public final class TimestampToIsbn13 {
    ...
    /**
     * Returns an ISBN‑13 style string made from the current timestamp (epoch milliseconds).
     *
    ...
    public static String currentIsbn() {
        ...
        return epoch.substring(0, 3)
                + "-" // 999
                + epoch.substring(3, 4)
                + "-" // 9
                + epoch.substring(4, 6)
                + "-" // 99
                + epoch.substring(6, 12)
                + "-" // 123456
                + epoch.substring(12); // 2
    }

    /**
     * Same as {@link #currentIsbn()}, but you can supply your own {@code epochMillis} value. Useful
