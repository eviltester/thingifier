...
public class C036PutCreateNewChallengerSessionTest extends RestAssuredBaseTest {
    ...
    public void canPutChallengerSessionToCreate() {
        ...
        Challenger challengerResponse =
                new Gson().fromJson(aresponse.body().asString(), Challenger.class);

        String aNewGuid = UUID.randomUUID().toString();

        int expectedStatusCode = 201; // 201 if multi user mode
        boolean singleUserMode =
                challengerResponse.xChallenger.equals("rest-api-challenges-single-player");
        if (singleUserMode) {
        ...
        */
        ...
        if (!singleUserMode) {
            Assertions.assertTrue(
                    statuses.getChallengeNamed("PUT /challenger/guid CREATE").status,
