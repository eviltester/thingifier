...
class RelationshipsTest {
    ...
    void canCreateAThingInstanceRelationships() {
        ...
        final EntityInstanceRelationships relationships =
                new EntityInstanceRelationships(fromInstance);

        System.out.println(relationships.toString());

        Assertions.assertNull(relationships.getTypeOfConnectableItems("bob"));
        Assertions.assertTrue(relationships.getConnectedItems("bob").isEmpty());
        Assertions.assertTrue(relationships.getConnectedItemsOfType("bob").isEmpty());
    ...
    void canConnectToThingWhenRelationshipIsDefined() {
        ...
        Assertions.assertFalse(toInstance.getRelationships().hasAnyRelationshipInstances());

        final Collection<EntityInstance> instances = relationships.getConnectedItems("fromto");
    ...
    void cannotConnectToThingWhenNoRelationshipIsDefined() {
        ...
        final IllegalArgumentException e =
                Assertions.assertThrows(
                        ...
                        () -> relationships.connect("unknownrelationship", toInstance));

        Assertions.assertTrue(e.getMessage().contains("Unknown Relationship"), e.getMessage());
    }
    ...
    void cannotConnectToThingWhenRelationshipIsNotForTypePassedIn() {
        ...
        final EntityInstance instance =
                thingfrom.addInstance(new EntityInstance(thingfrom.definition()));

        final IllegalArgumentException e =
                Assertions.assertThrows(
                        IllegalArgumentException.class,
                        () -> relationships.connect("fromto", instance));
    ...
    void canCreateATwoWayConnection() {
        ...
        Assertions.assertTrue(toInstance.getRelationships().hasAnyRelationshipInstances());
    }
    ...
    void removeRelationshipsBasedOnName() {
        ...
        relationships.removeRelationshipsInvolving(toInstance, "fromto");
    ...
    void removeAllRelationshipsTwoWayReversed() {
        ...
        fromInstance.getRelationships().connect("fromto", toInstance);

        Assertions.assertTrue(fromInstance.getRelationships().hasAnyRelationshipInstances());

        final List<EntityInstance> thingsToDelete =
                toInstance.getRelationships().removeAllRelationships();

        Assertions.assertFalse(fromInstance.getRelationships().hasAnyRelationshipInstances());
