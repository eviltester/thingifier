...
class AutoIncrementIdFieldTest {
    ...
    void normalValidateAgainstTypeForIdDoesNotAllowSetting() {
        ...
        final ValidationReport report = field.validate(FieldValue.is(field, "1"));
    ...
    void exampleIDIsAnIntegerBetween_1_and_100() {
        ...
        for (int x = 0; x < 100; x++) {
            ...
            Assertions.assertTrue(
                    exampleAsInteger >= 1,
                    "example id should be greater than or equal to 1 but was " + exampleAsInteger);

            Assertions.assertTrue(
                    exampleAsInteger <= 100,
                    "example id should be less than or equal to 100 but was " + exampleAsInteger);
        }
    }
}
