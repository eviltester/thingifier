...
public class ProjectTodoRelationshipCrudTest {
    ...
    public static void clearDataFromEnv() {
        ...
        when().post("/admin/data/thingifier").then().statusCode(200);

        final JsonPath clearedData =
                when().get("/todos").then().statusCode(200).extract().body().jsonPath();
    ...
    public void canCreateAndAmendSequence() {
        ...
        given().body(givenRefBody)
                .contentType("application/json")
                .when()
                .post("/projects/" + specificProjectId + "/tasks")
                .then()
                .statusCode(201)
                .contentType(ContentType.JSON);
        ...
        Response response =
                when().get("/projects/" + specificProjectId + "/tasks")
                        .then()
                        .statusCode(200)
                        .contentType(ContentType.JSON)
                        .and()
        ...
        Assertions.assertEquals(
                ...
                response.getBody()
                        .jsonPath()
                        .get("todos[0].relationships[0].tasksof[0].projects[0].id"));
        // Assertions.assertEquals(specificProjectGuid,
        // response.getBody().jsonPath().get("todos[0].task-of[0].guid"));
        ...
        response =
                when().get("/projects/" + specificProjectId)
                        .then()
                        .statusCode(200)
                        .contentType(ContentType.JSON)
                        .and()
