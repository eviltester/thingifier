...
public class ChallengeApiModelRepositoryTest {
    ...
    public void challengeApiGetUsesSqliteRepositoryReadsForSimpleTodoQueries() {
        try (SqliteThingRepositoryProvider provider = SqliteThingRepositoryProvider.inMemory()) {
            ...
            repository.patchInstance(
                    firstTodo, EntityInstanceDraft.forEntity(todo).withField("doneStatus", "true"));
    ...
    public void simpleEntityWritesUseSqliteRepositoryWithoutLoadingCompatibilitySnapshot() {
        try (SqliteThingRepositoryProvider provider = SqliteThingRepositoryProvider.inMemory()) {
            ...
            Assertions.assertEquals(
                    "posted",
                    repository
                            .findInstanceByQueryIdentifier(note, "1")
                            .getFieldValue("title")
                            .asString());
    ...
    public void sqliteMemoryProvidersHaveSeparateNamedDatabaseLifetimes() {
        ...
        try (SqliteThingRepositoryProvider firstProvider =
                        SqliteThingRepositoryProvider.inMemory();
                SqliteThingRepositoryProvider secondProvider =
                        SqliteThingRepositoryProvider.inMemory()) {
            ...
            Assertions.assertEquals(
                    ...
                    firstModel
                            .getRepository(EntityRelModel.DEFAULT_DATABASE_NAME)
                            .listInstances(firstNote)
                            .size());
            Assertions.assertEquals(
                    0,
                    secondModel
                            .getRepository(EntityRelModel.DEFAULT_DATABASE_NAME)
                            .listInstances(secondNote)
                            .size());
        } finally {
            if (firstModel != null) {
                firstModel.close();
