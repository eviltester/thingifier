...
public class AwsS3Storage implements ChallengerPersistenceMechanism {
    ...
    public PersistenceResponse saveChallengerStatus(final ChallengerAuthData data) {
        ...
        return new PersistenceResponse()
                .withSuccess(false)
                .withErrorMessage("AWS not supported by current build");

        //        // by default will not save to aws - need to add environment variable
        //        if(!allowSave){
        //            return new PersistenceResponse().withSuccess(false).withErrorMessage("AWS
        // Configuration does not allow saving challenger status");
        //        }
        //
        //        if(data==null){
        //            return new PersistenceResponse().withSuccess(false).withErrorMessage("no data
        // provided");
        //        }
        //
        //
        ...
        //        } catch (Exception e) {
        //
        //            logger.error("Error storing data to bucket for guid: {}",
        // data.getXChallenger(), e);
        //            return new PersistenceResponse().withSuccess(false).withErrorMessage("Error
        // storing data to S3");
        //        }
    }
    ...
    public PersistenceResponse loadChallengerStatus(final String guid) {
        ...
        //        if(!allowLoad){
        //            return new PersistenceResponse().withSuccess(false).withErrorMessage("AWS
        // Configuration does not allow loading challenger status");
        //        }
        //
        //        try {
        //            ensureClientExists();
        //
        //            final S3Object fullObject = s3Client.getObject(new
        // GetObjectRequest(bucketName, guid));
