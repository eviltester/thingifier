...
public class Tasks_ProjectToTodosTest {
    ...
    void createTaskRelationshipUsingUrl() {
        ...
        Payloads.TodoPayload createdTodo = todoCreation.body().as(Payloads.TodoPayload.class);

        // {"id":"%s"}
        final Response crossRefResponse =
                RestAssured.given()
                        .contentType(ContentType.JSON)
                        .body(String.format("{\"id\":\"%s\"}", createdTodo.id))
                        .when()
                        .post(
                                Environment.getEnv(
        ...
        // check project is cross referenced to todoinstance
        final Response checkTodo = RestAssured.get(Environment.getEnv("/todos/" + createdTodo.id));
    ...
    void createTaskRelationshipUsingComplexField() {
        ...
        final Response crossRefResponse =
                RestAssured.given()
                        .contentType(ContentType.JSON)
                        .body(String.format("{\"tasks\": [{\"id\":\"%s\"}]}", createdTodo.id))
                        .when()
                        .post(Environment.getEnv(String.format("/projects/%s", createdProject.id)))
                        .andReturn();
